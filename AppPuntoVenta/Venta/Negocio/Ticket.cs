...
namespace AppPuntoVenta
{
    public class Ticket
    {
        ...
        float AjustarImgaen(Image imagen, ref float width, ref float height, int maxSize, int pageSize)
        {
            float posicionX = 0;
            float porcAjuste = 0;
            ...
            if (imagen.Width >= imagen.Height)
            {
                ...
                {
                    porcAjuste = (maxSize * 100) / imagen.Width;
                    width = maxSize;
                    height = imagen.Height * (porcAjuste / 100);
                }
            }else if (imagen.Height > imagen.Width)
            {
                if (imagen.Height > maxSize)
                {
                    porcAjuste = (maxSize * 100) / imagen.Height;
                    height = maxSize;
                    width = imagen.Width * (porcAjuste / 100);
                }
            }
            posicionX = (pageSize / 2) - (width / 2);
            posicionX += 10;
            return posicionX;
        }
        ...
        void pdoc_PrintPage(object sender, PrintPageEventArgs e)
        {
            ...
            if (File.Exists(Sesion.RutaLogo))
            {
                using (FileStream archivoLogo = new FileStream(Sesion.RutaLogo, FileMode.Open))
                {
                    Image logo = Image.FromStream(archivoLogo);
                    float wLogo = 0;
                    float hLogo = 0;
                    float posicionX = AjustarImgaen(logo,ref wLogo,ref hLogo, 100, 270);
                    graphics.DrawImage(logo, posicionX, 0,wLogo, hLogo);
                    Offset = Offset + (int)hLogo +10;
                }
            }
